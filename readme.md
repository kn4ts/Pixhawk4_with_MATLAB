...
## フライトコントローラのコード
### `flight_cont_PID_cascade_altitude(mock)/`
※注意1...プロポ送信機の設定で`Ch6`，`Ch7`，`VR`を割り当ててから使用すること．  
	＜プロポの設定方法＞  
	1. 「＋」ボタンを長押し．
...
※注意2...`Ch6`，`Ch7`のスイッチは一番奥まで倒し，`Arm`中は操作しないこと（試用段階の高度制御モード用のスイッチのため）．  
※注意3...`Arm`状態になると各制御器中の積分器が動作を始めるため，ワインドアップ現象に注意すること（`Disarm`でリセットされる）．
