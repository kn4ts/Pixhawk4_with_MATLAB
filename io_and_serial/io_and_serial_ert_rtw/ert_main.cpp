...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(io_and_serial_M) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    io_and_serial_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(io_and_serial_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(io_and_serial_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(io_and_serial_M, 0);
